...
SIGNATURES: list[tuple[str, str, str, str]] = [
    ...
    (
        "usc_header_cell",
        r"\|\s*University of Southern California",
        "critical",
        "site header table cell captured as content",
    ),
    (
        "responsive_markers",
        r"Begin Responsive|End Responsive",
        "critical",
    ...
    (
        "literal_html_tag",
        r"<\s*(?:div|span|td|tr|table|ul|li|p|a|img|link|style|body|html|meta)\b[^>]*>",
        "critical",
        "literal HTML markup present as text",
    ),
    ("js_filename", r"\b[\w/.-]+\.js\b", "high", "JavaScript filename in text"),
    ("css_filename", r"\b[\w/.-]+\.css\b", "high", "CSS filename in text"),
    ("html_entity", r"&(?:nbsp|amp|lt|gt|quot|#\d+);", "medium", "unresolved HTML entity"),
    # --- generic loading / error / bot-wall states ---
    (
        "loading_text",
        r"\b(?:Loading\.\.\.|Please wait|Just a moment)\b",
        "critical",
        "loading placeholder text",
    ),
    (
        "js_error_text",
        r"(?:ReferenceError|TypeError:|undefined is not|Uncaught )",
        "critical",
        "JavaScript error text",
    ),
    (
        "waf_text",
        r"(?:Request unsuccessful|Incapsula|verify you are human|Access Denied|"
        r"Pardon our interruption|captcha)",
        "critical",
        "bot-wall / access-denied text",
    ),
    (
        "cookie_consent",
        r"(?:accept all cookies|cookie policy|consent preferences)",
        "medium",
        "cookie/consent banner text",
    ),
    (
        "nav_menu_run",
        r"Courses\s+Programs\s+School",
        "high",
        "navigation menu run-on (nav captured as content)",
    ),
    (
        "noscript_banner",
        r"Javascript is currently not supported",
        "medium",
...
def audit_corpus(corpus: Path) -> list[dict]:
    ...
    if fl.exists():
        for row in csv.DictReader(fl.open(encoding="utf-8")):
            ...
            already_ok = fetch_by_poid.get(poid, {}).get("result") == "ok"
            if row.get("result") == "ok" or (poid not in fetch_by_poid and not already_ok):
    ...
    for path in sorted(programs.glob("*.txt")):
        ...
        hits: dict[str, int] = {}
        excerpts: list[str] = []
        for key, pattern, _severity, _desc in SIGNATURES:
        ...
        title_mismatch = bool(
            expected_title
            and prog_name
            and norm_for_hash(expected_title) != norm_for_hash(prog_name)
        )
...
def stats_block(records: list[dict]) -> dict:
    ...
    q1, q3 = (
        (statistics.quantiles(counts_nz, n=4)[0], statistics.quantiles(counts_nz, n=4)[2])
        if len(counts_nz) >= 4
        else (counts_nz[0], counts_nz[-1])
    )
    iqr = q3 - q1
    ...
    out.update(
        {
            "min_chars": counts_nz[0],
            "max_chars": counts_nz[-1],
            "mean_chars": round(mean, 1),
            "median_chars": median,
            "stdev_chars": round(sd, 1),
            "q1": q1,
            "q3": q3,
            "iqr": iqr,
            "iqr_lower_fence": round(lo, 1),
            "iqr_upper_fence": round(hi, 1),
            "largest_file": {"name": nz[-1]["file_name"], "chars": nz[-1]["char_count"]}
            if nz
            else {},
            "five_smallest_nonzero": [
                {"name": r["file_name"], "chars": r["char_count"], "status": r["validation_status"]}
                for r in nz[:5]
            ],
            "five_largest": [
                {"name": r["file_name"], "chars": r["char_count"], "status": r["validation_status"]}
            ...
            "extreme_z_scores": [
                ...
                for r in nz
                if sd and abs((r["char_count"] - mean) / sd) >= 3
            ],
        }
    )
    return out
...
def main() -> int:
    ...
    for r in records:
        r["manual_review_priority"] = (
            ...
            if r["critical_signatures"] or r["high_signatures"]
            else 3
            if r["validation_status"] == "REVIEW"
            else 9
        )
    ...
    per_corpus = {
        c: stats_block([r for r in records if r["corpus"] == c])
    ...
    md = [
        f"# Scraper output audit — {len(records)} TXT files",
        "",
            ...
            md_table(
                [
                    [
                        r["file_name"],
                        r["char_count"],
                        r["http_status_accepted"],
                        r["critical_signatures"] or "-",
                        r["max_repeated_sentence_count"],
                    ]
                    for r in sorted(fails, key=lambda x: x["file_name"])
                ],
    ...
    md.append(
        "Largest: "
        + ", ".join(f"{r['name']} ({r['chars']}, {r['status']})" for r in overall["five_largest"])
    )
    md += [
        "",
        "## Statistical outliers (overall)",
        "",
    ...
    for r in sorted(fails, key=lambda x: x["file_name"]):
        print(
            f"  FAIL {r['file_name']:<60} chars={r['char_count']:<7} http={r['http_status_accepted']} "
