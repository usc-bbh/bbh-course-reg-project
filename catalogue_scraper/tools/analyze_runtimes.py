...
def main() -> int:
    ...
    def fmt(p: dict) -> str:
        return (f"{p['corpus']}/{p['page']:>3} {p['program'][:42]:<44} "
    ...
    A(f"- Per-page rows parsed: **{len(pages)}** "
    ...
    A("")
    A("Note from the workbook's own Overview sheet: the majors run completed all 207 "
    ...
    A("")
    A(f"| n | min | Q1 | median | mean | Q3 | IQR | 1.5×IQR fence | max | stdev |")
    A("|---|---|---|---|---|---|---|---|---|---|")
    A(f"| {len(secs)} | {secs[0]:.0f}s | {q1:.0f}s | {med:.0f}s | {mean:.1f}s | {q3:.0f}s | "
    ...
    if fails and passes:
        ...
        A(f"- FAIL pages (n={len(fs)}): median **{statistics.median(fs):.0f}s**, "
          f"mean {statistics.mean(fs):.1f}s")
        A(f"- PASS pages (n={len(ps)}): median **{statistics.median(ps):.0f}s**, "
          f"mean {statistics.mean(ps):.1f}s")
        slow_fail = sum(1 for p in outliers if p["validation_status"] == "FAIL")
        A(f"- Of the {len(outliers)} slowest-outlier pages, **{slow_fail}** produced a FAIL output "
          f"({100 * slow_fail / max(1, len(outliers)):.0f}%).")
        fast_fail = sum(1 for p in fails if p["seconds"] <= med)
        A(f"- **{fast_fail} of {len(fails)}** FAIL pages ran at or below the median runtime — "
          "i.e. most contaminated outputs came from *fast* scrapes.")
        A("")
        A("**Conclusion: runtime does not predict contamination.** Slow pages are explained by "
    ...
    A("")
    for key, label in (("corpus", "corpus"), ("method", "acquisition method"),
                       ("degree", "degree type")):
        ...
        for b, ps in sorted(buckets.items(), key=lambda kv: -len(kv[1]))[:10]:
            ...
            A(f"| {b} | {len(ps)} | {nf} | {100 * nf / len(ps):.0f}% | "
    ...
    if target:
        ...
        A(f"- Sheet row: page {target['page']} ({target['corpus']}), "
          f"method `{target['method']}`, **{target['seconds']:.0f}s**")
        A(f"- z-score vs all pages: **{z:+.2f}** — "
          f"{'a statistical outlier' if abs(z) >= 2 else 'within the normal range'}")
        A(f"- Percentile: **{100 * sum(1 for s in secs if s <= target['seconds']) / len(secs):.0f}th**")
        A("")
        A("It was slow (browser escalation after the bot-wall), but *not* uniquely slow, and "
    ...
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
    (out_md.parent / "runtime_pages.json").write_text(
