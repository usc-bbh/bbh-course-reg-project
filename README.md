...
| D-clearance | `validator/data/dept_clearance.json` (`_schema_version`); provenance in `validator/data/DEPT_CLEARANCE_SOURCES.md` |
| Requirement constraint types | `constraint_classifier_prompt.md` §6, Taxonomy reference |

## `validator/data/` — static reference data
...
The `validator/data/` folder is different: it holds **static, hand-curated
information we pulled once and reference** rather than regenerate per semester.
It lives under `validator/` because the validator is its only consumer.
...
`validator/data/DEPT_CLEARANCE_SOURCES.md` documents the provenance of every
entry in `validator/data/dept_clearance.json`. The validator loads
`validator/data/dept_clearance.json` at runtime to warn when a planned course
needs D-clearance.
